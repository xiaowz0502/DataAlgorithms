...
String& String::erase(size_t pos, size_t len) {
    ...
    size_t removed = pos;
    size_t newBegin = pos + len;
    
    if (len <= sz - pos){
    
    if (len != npos){
        for (size_t i = newBegin; i <= sz; i++){
            cstr[removed] = cstr[i];
            removed ++;
         }
    }
    else if (len == npos){
        cstr[removed] = a_null_byte;
    }
    sz = sz - len;
    }
    else{
...
String& String::insert(size_t pos, const String& str) {
    ...
    else if (pos > 0 && pos <sz - add){
        String all;
        String first;
        String second;
        
        first = this->substr(0,pos);
        second = this->substr(pos, sz - pos);
        all += first;
        all += str;
        all += second;
        this->swap(all);
...
size_t String::find_first_of(const String& str, size_t pos) {
    ...
    return npos;
    // GET RID OF THIS LINE WHEN YOU BEGIN IMPLEMENTING FIND_FIRST_OF (this is for compile purposes only)
}
...
size_t String::find_last_of(const String& str, size_t pos) {
    ...
    return npos;
        // GET RID OF THIS LINE WHEN YOU BEGIN IMPLEMENTING FIND_LAST_OF (this is for compile purposes only)
}
